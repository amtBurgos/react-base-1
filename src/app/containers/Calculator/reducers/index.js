...
const inputOperator = (state, action) => {
  ...
  const result = newValue ? calculate(prevOperator, prevValue, nextValue) : prevValue;
...
const inputOperation = (state, action) => {
  ...
  switch (operation) {
    case ActionTypes.PERCENT:
      value = prevValue / 100;
      return state.set('display', value).set('prevValue', value);
...
const result = (state) => {
  ...
  const prevValue = state.get('prevValue');
  const nextValue = state.get('nextValue');
  const total = calculate(operator, prevValue, nextValue);
